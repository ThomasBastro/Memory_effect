...
def memory_afterglow(E_aft, theta_ej, phi_ej, d, beta=0.99):
    """
    Memory from the afterglow phase with the same formula as the memory of the prompt phase
    but with the total energy in the afterglow phase instead of the kinetic energy of the jet.
    Parameters:
    -----------
    E_aft : float
        Total energy in the afterglow phase [erg]
    theta_ej : float
        Viewing angle [rad]
    phi_ej : float
        Azimuthal angle of the jet in the plane of the sky [rad]
    d : float
        Distance to the source [pc]
    beta : float
        Normalized velocity of the jet (v/c), typically close to 1 for
    
    Returns:
    ---------
    h_aft : float
        Amplitude of the memory from the afterglow phase
    """
    # Angular factor (same as initial memory)
    angular_factor = beta**2 * np.sin(theta_ej)**2 * np.cos(2 * phi_ej) / (1 - beta* np.cos(theta_ej))  
    # Convert energy from erg to Joules
    ...
    # Convert distance from parsec to meters
    d_m = d * PC_SI
    # Calculate the memory amplitude from the afterglow phase
    h_aft = (G_SI / C_SI**4) * (E_aft / d_m) * angular_factor
    
    return h_aft

def memory_total_waveform(t_obs, h_in, h_m, t_end_injection, theta_ej, d, radius =0.01):
    """
    ...
    """
    z = z_at_value(Planck18.luminosity_distance, d * u.pc).value  # Convert distance to redshift 
    ...
    t_m = t_end_injection + ( (radius * PC_SI) * (1-np.cos(theta_ej)) * (1+z) )/ C_SI 
...
def total_waveform_fft(h_in, h_m, t_end_injection, theta_ej, d, radius=0.01):
    """
